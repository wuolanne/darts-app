...
export function CheckoutRouteSummary({
  ...
  const routeData = useMemo(() => {
    ...
    const primaryPathFormatted = formatRoute(primaryPath);
    const primaryQuality =
      rankedRoutes.find((route) => formatRoute(route.path) === primaryPathFormatted)?.quality ??
      bestRoute?.quality ??
      null;
    ...
    const topAlternative = curatedAlternatives[0]
      ...
      : rankedRoutes.find((route) => formatRoute(route.path) !== primaryPathFormatted)
        ? {
            route: rankedRoutes.find((route) => formatRoute(route.path) !== primaryPathFormatted)!.path,
            quality: rankedRoutes.find((route) => formatRoute(route.path) !== primaryPathFormatted)!.quality
          }
        : null;
